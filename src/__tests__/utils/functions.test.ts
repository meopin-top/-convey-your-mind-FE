...
describe("compose", () => {
  it("입력으로 받은 함수를 왼쪽부터 차례대로 실행해야 한다.", () => {
    ...
    // then
    expect(consoleSpy.mock.calls[0][0]).toEqual("function1 실행")
    expect(consoleSpy.mock.calls[1][0]).toEqual("function2 실행")
    expect(consoleSpy.mock.calls[2][0]).toEqual("function3 실행")
